...
class SSDB
{
	...
	function _connect() {
		$timeout_f = (float)($this->_timeout_ms)/1000;
		$socket_addr = $this->_host . ":" . $this->_port;
		$this->sock = @stream_socket_client($socket_addr, $errno, $errstr, $timeout_f);
		if(!$this->sock){
			throw new SSDBException("$errno: $errstr");
		}
		$timeout_sec = intval($this->_timeout_ms/1000);
		$timeout_usec = ($this->_timeout_ms - $timeout_sec * 1000) * 1000;
	...
	function multi_zset($name, $kvs=array()){
		...
		foreach($kvs as $k=>$v){
			...
			{
				if (isset($v['score']) && isset($v['value']) )
				{
					$args[] = $k;
					$args[] = $v['score'];
					$args[] = $v['value'];
		...
		switch($cmd){
			...
			case 'zget':
				if($resp[0] == 'ok')
				{
					if(count($resp) == 3)
					{
						$data = array('score'=>$resp[1], 'value'=>$resp[2]);
			...
			case 'zrrange':
				if($resp[0] == 'ok'){
					...
					{
						$data = array();
						for($i=1; $i<count($resp); $i+=3){
							$data[$resp[$i]] = array('score'=>$resp[$i + 1], 'value'=>$resp[$i + 2]);
						}
						return new SSDB_Response('ok', $data);
					}else{
						return new SSDB_Response('server_error', 'Invalid response');
